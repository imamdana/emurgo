    ...
    VotingContract.deployed().then(function(instance){
      ...
      instance.getNumOfCandidates().then(function(numOfCandidates){
        ...
        if (numOfCandidates == 0){
          ...
          instance.addCandidate("Anies Baswedan", "NasDem").then(function(result){
            $("#candidate-box").append(`<div class='form-check'><input class='form-check-input' type='checkbox' value='' id=${result.logs[0].args.candidateID}><label class='form-check-label' for=0>Anies Baswedan</label></div>`)
          })
          instance.addCandidate("Ganjar Pranowo", "PSI").then(function(result){
            $("#candidate-box").append(`<div class='form-check'><input class='form-check-input' type='checkbox' value='' id=${result.logs[0].args.candidateID}><label class='form-check-label' for=1>Ganjar Pranowo</label></div>`)
          })
          instance.addCandidate("Prabowo Subianto", "Gerindra").then(function(result){
            $("#candidate-box").append(`<div class='form-check'><input class='form-check-input' type='checkbox' value='' id=${result.logs[0].args.candidateID}><label class='form-check-label' for=2>Prabowo Subianto</label></div>`)
          })
          ...
          console.log('fetching candidates, total: ' + numOfCandidates);
          for (var i = 0; i < numOfCandidates; i++ ){
            console.log(i);
            // gets candidates and displays them
            instance.getCandidate(i).then(function(data){
              console.log(`i: ${i}, data:`, data);
              $("#candidate-box").append(`<div class="form-check"><input class="form-check-input" type="checkbox" value="" id=${data[0]}><label class="form-check-label" for=${data[0]}>${data[1]}</label></div>`)
            }).catch(function(e){
              console.log(e);
            });
          }
    ...
    VotingContract.deployed().then(function(instance){
      ...
      for (var i = 0; i < window.numOfCandidates; i++){
        ...
        Promise.all([candidatePromise,votesPromise]).then(function(data){
          box.append(`<p>${data[0][1]}: ${data[1]}</p>`)
        }).catch(function(err){ 
          console.error("ERROR! " + err.message)
        })
      }
